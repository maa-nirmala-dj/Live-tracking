...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Outfit', sans-serif; -webkit-tap-highlight-color: transparent; outline: none; }
        ...
        /* Advanced Location Display (Village Name) */
        ...
        .timeline-feed { padding-left: 20px; position: relative; flex-grow: 1; overflow-y: auto; padding-right: 5px; min-height: 200px; }
...
<body>
    ...
    <div id="view-admin-session" class="view-container">
        ...
        <div class="card flex-grow" style="padding-bottom:10px;">
            ...
            <textarea id="admin-update-text" class="mn-input" rows="2" style="resize:none; padding:15px; flex-shrink:0;" placeholder="Or type a custom update here..."></textarea>
            <button class="mn-btn btn-gold" style="margin-top:12px; margin-bottom:25px; flex-shrink:0; background: linear-gradient(135deg, var(--neon-cyan) 0%, #0088cc 100%); box-shadow: 0 5px 25px rgba(0,229,255,0.3);" id="btn-push-update" onclick="adminPushUpdate()"><i class="fas fa-paper-plane"></i> TRANSMIT TO TARGET</button>
            
            <!-- Admin Manage Active Updates -->
            <div style="flex-grow:1; display:flex; flex-direction:column; border-top: 1px dashed rgba(255,255,255,0.1); padding-top:15px;">
                <h4 style="font-size:12px; color:#aaa; margin-bottom:10px; text-transform:uppercase; flex-shrink:0;">Manage Sent Updates:</h4>
                <div id="admin-manage-updates-area" class="timeline-feed">
    ...
    <div id="view-client" class="view-container" style="padding: 20px 10px;">
        ...
        <div class="card flex-grow" style="max-width:100%; width:100%; display:flex; flex-direction:column;">
            <h3 style="margin-bottom: 20px; flex-shrink:0;"><i class="fas fa-bolt"></i> Logistics Comms Feed</h3>
        ...
        <button class="mn-btn btn-danger" style="max-width:100%; width:100%; flex-shrink:0;" onclick="systemLogout()"><i class="fas fa-power-off"></i> DISCONNECT TERMINAL</button>
    </div>
    ...
    <script>
        ...
        function processLogin() {
            ...
            db.ref('trackings/' + phone).once('value').then((snapshot) => {
                ...
                if (snapshot.exists()) {
                    ...
                    if(clientData.pin === pin || !clientData.pin) { // Fallback if pin missing
                        document.getElementById('login-phone').value = ''; document.getElementById('login-pin').value = '';
                        ...
                        // Reset Notification Engine
                        processedUpdateKeys.clear();
                        isInitialLoad = true; 

                        startClientWatchListeners(); 
                        switchView('view-client');
                        showToast(`Welcome, ${clientData.name || 'Client'}`);
                    } else {
            ...
            db.ref('trackings').orderByChild('timestamp').on('value', (snapshot) => {
                if(snapshot.exists()) {
                    ...
                    snapshot.forEach(child => {
                        // Ensure required fields exist to prevent undefined rendering
                        let data = child.val();
        ...
        // --- 11. CLIENT: SHARE LOCATION TO HQ ---
        ...
        // --- 12. CLIENT: TARGETED WATCH LISTENERS & PUSH NOTIFICATIONS ---
        ...
        function startClientWatchListeners() {
            ...
            // Ultimate Push Notification Engine & Timeline Sync
            db.ref(`trackings/${currentClientPhone}/updates`).orderByChild('timestamp').limitToLast(100).on('value', (snapshot) => {
                ...
                if(snapshot.exists()) {
                    ...
                    snapshot.forEach(child => {
                        ...
                        // Fire notification if it's new
                        if (!isInitialLoad && !processedUpdateKeys.has(key)) {
                            showPushNotification("MND Logistics Update", updateData.text);
                        }
